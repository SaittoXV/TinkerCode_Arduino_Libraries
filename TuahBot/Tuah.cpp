...
void tuah::knightrider(int delayx)
{
  digitalWrite(led1, HIGH);
  delay(delayx);
...
void tuah::RelaySwitch(int delayx)
{
...
void tuah::MazeRunner (int speed)
{
...
void tuah::motortank(char *directionL,  int powerL, char *directionR,int powerR)
{
...
void tuah::motorsteer(char* robotdirection, int power)
{
...
// or if you just want the emitters on all the time and don't want to
// use an I/O pin to control it, use a value of 255 (QTR_NO_EMITTER_PIN).
void tuah::init(unsigned char* pins,
    unsigned char numSensors, unsigned char numSamplesPerSensor,
    unsigned char emitterPin)
{
    QTRSensors::init(pins, numSensors, emitterPin);
...
void tuah::readPrivate(unsigned int *sensor_values)
{
    unsigned char i, j;
...
void QTRSensors::tuahtillJunction(int lajul, int lajur, int speedturn, int sensorc, int inByte, float kp, float kd,int linef,int basespeed) 
...
void QTRSensors::back_off(char a, char b)
{
  analogWrite (_pin1,a);
