/*
  Original code from tuahMini.h - Library for Tuah Mini
  Copyright (c) 2018 Fauzan Khairi Che Harun.  All right reserved.
  
  Edited to comply with Mechabot Rush v3
...
mechabotrushv2::mechabotrushv2(unsigned char* pins,
...
void mechabotrushv2::motortank(char *directionL,  int powerL, char *directionR,int powerR)
{
...
void mechabotrushv2::motorsteer(char* robotdirection, int power)
{
...
// or if you just want the emitters on all the time and don't want to
// use an I/O pin to control it, use a value of 255 (QTR_NO_EMITTER_PIN).
void mechabotrushv2::init(unsigned char* pins,
    unsigned char numSensors, unsigned char numSamplesPerSensor,
    unsigned char emitterPin)
{
    QTRSensors::init(pins, numSensors, emitterPin);
...
void mechabotrushv2::readPrivate(unsigned int *sensor_values)
{
    unsigned char i, j;
...
void QTRSensors::mechabotrushv2tillJunction(int lajul, int lajur, int speedturn, int sensorc, int inByte, float kp, float kd,int linef,int maxSpeed, int forwarddelay) 
...
void QTRSensors::mechabotrushv2tillJunctionSimple(int laju, int inByte, float kp, float kd, int linef) 
...
/* 
void QTRSensors::mechabotrushv2FinderB(int inByte, int inNext, int lajur, int lajul, float turn, int speedturn, float kp, float kd, int sensorc, int lajak, int dturn) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::mechabotrushv2FinderWhite(int inByte, int inNext, int laju,int turnspeed, float kp, float kd,  int terus,int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
//white background, black line
void QTRSensors::mechabotrushv2FinderBlack(int inByte, int inNext, int laju, int turnspeed, float kp, float kd, int terus, int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::mechabotrushv2Dtracer(int lajur, int lajul, int speedturn, float kp, float kd, int sensorc, int linef, int dtracer) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::mechabotrushv2flineW(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
...
void QTRSensors::mechabotrushv2flineB(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
	sensorc =(calibratedMaximumOn[2]+calibratedMinimumOn[2])/2;
    position = readLine(sensorValues,1,linef); //0 black on white, 1 white on black
...
// LEDsense(position); 
  ...
  if (left_motor_pwm < 0)
    ...
    left_motor_pwm=constrain(left_motor_pwm,0,maxSpeed);
      ...
      advance(left_motor_pwm,right_motor_pwm);
