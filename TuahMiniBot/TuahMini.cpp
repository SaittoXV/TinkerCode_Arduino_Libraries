/*
  tuahMini.h - Library for Tuah Mini
...
tuahmini::tuahmini(unsigned char* pins,
...
void tuahmini::knightrider(int delayx)
{
  digitalWrite(led1, HIGH);
  delay(delayx);
...
int tuahmini::ultrasonic()
{


}

void tuahmini::RelaySwitch(int delayx)
{
...
void tuahmini::MazeRunner (int speed)
{
...
void tuahmini::motortank(char *directionL,  int powerL, char *directionR,int powerR)
{
...
void tuahmini::motorsteer(char* robotdirection, int power)
{
...
void tuahmini::megablink(int del)
{
  digitalWrite(led1, LOW);
  digitalWrite(led2, LOW);
  digitalWrite(led3, LOW);
...
void tuahmini::LED(int l,char *stat)
{
...
int tuahmini::ir_sense(int l)
{
int sensordata= analogRead(l);
return sensordata;
}

int tuahmini::Pbutton()
{
...
// or if you just want the emitters on all the time and don't want to
// use an I/O pin to control it, use a value of 255 (QTR_NO_EMITTER_PIN).
void tuahmini::init(unsigned char* pins,
    unsigned char numSensors, unsigned char numSamplesPerSensor,
    unsigned char emitterPin)
{
    QTRSensors::init(pins, numSensors, emitterPin);
...
void tuahmini::readPrivate(unsigned int *sensor_values)
{
    unsigned char i, j;
...
void QTRSensors::tuahtillJunction(int lajul, int lajur, int speedturn, int sensorc, int inByte, float kp, float kd,int linef,int maxSpeed, int forwarddelay) 
...
void QTRSensors::tuahtillJunctionSimple(int laju, int inByte, float kp, float kd, int linef) 
...
void QTRSensors::tuahFinderW(int inByte, int inNext, int lajur, int lajul, float turn, int speedturn, float kp, float kd, int sensorc, int lajak, int dturn) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::tuahFinderB(int inByte, int inNext, int lajur, int lajul, float turn, int speedturn, float kp, float kd, int sensorc, int lajak, int dturn) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::tuahFinderBlack(int inByte, int inNext, int laju,int turnspeed, float kp, float kd,  int terus,int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::tuahFinderWhite(int inByte, int inNext, int laju, int turnspeed, float kp, float kd, int terus, int turndelay) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::tuahDtracer(int lajur, int lajul, int speedturn, float kp, float kd, int sensorc, int linef, int dtracer) // (type of junction, next robot action, robot speed, turn speed percentage)
{
...
void QTRSensors::tuahflineW(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
...
void QTRSensors::tuahflineB(int lajur, int lajul, int sensorc, int linef) // (right speed, left speed, sensor, line format)
{
	sensorc =(calibratedMaximumOn[2]+calibratedMinimumOn[2])/2;
    position = readLine(sensorValues,1,linef); //0 black on white, 1 white on black
